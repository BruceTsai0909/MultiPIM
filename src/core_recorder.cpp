...
class TimingCoreEvent : public TimingEvent {
    ...
    public:
        ...
        void simulate(uint64_t _startCycle) {
            //Hack by CY
            if(_startCycle < origStartCycle){
                this->requeue(origStartCycle);
                return;
            }
...
uint64_t CoreRecorder::cSimEnd(uint64_t curCycle) {
    ...
    //Hack by CY
    if(lastEvCycle1 > lastEvCycle2)
        lastEvCycle2 = lastEvCycle1;
...
uint64_t CoreRecorder::getUnhaltedCycles(uint64_t curCycle) const {
    ...
    // printf("curCycle %ld,zinfo->globPhaseCycles %ld, cycle %ld, haltedCycles %ld, getUnhaltedCycles %ld\n", curCycle, zinfo->globPhaseCycles, cycle, haltedCycles, cycle - haltedCycles);
