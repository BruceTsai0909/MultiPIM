...
static const char* accessTypeNames[] = {"GETS", "GETX", "PUTS", "PUTX", "OFFLOAD"/* , "EVICTION", "SETDIRTY", "WRITEBACK" */};
static const char* invTypeNames[] = {"INV", "INVX"};
static const char* mesiStateNames[] = {"I", "S", "E", "M"};
static const char* ivStateNames[] = {"I", "V"};
static const char* writePolicyNames[] = {"WriteBack", "WriteThrough"};
...
const char* WritePolicyName(WritePolicy wp){
    assert_msg(wp >= 0 && (size_t)wp < sizeof(writePolicyNames)/sizeof(const char*), "WritePolicyName got an out-of-range input, %d", wp);
    return writePolicyNames[wp];
}

WritePolicy WritePolicyType(const char * name){
    std::string policyStr(name);
    if(policyStr == "WriteBack")
        return WRITEBACK;
