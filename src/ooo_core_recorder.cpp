...
class OOORespEvent : public TimingEvent {
    ...
    public:
        OOORespEvent(uint64_t preDelay, OOOCoreRecorder* _cRec, int32_t domain = -1) : TimingEvent(preDelay, 0, domain), cRec(_cRec), tid(-1), offload(false) {}
        OOORespEvent(uint64_t preDelay, OOOCoreRecorder* _cRec, uint32_t _tid, bool _offload, int32_t domain = -1) : 
                    TimingEvent(preDelay, 0, domain), cRec(_cRec), tid(_tid), offload(_offload) {}
...
void OOOCoreRecorder::addIssueEvent(uint64_t evCycle,  uint32_t tid) {
    assert(lastEvProduced);
    uint64_t zllCycle = evCycle - gapCycles;
    assert_msg(zllCycle >= lastEvProduced->zllStartCycle, "zllCycle %ld last %ld", zllCycle, lastEvProduced->zllStartCycle);
    OOOIssueEvent* ev = new (eventRecorder) OOOIssueEvent(0, zllCycle, this, domain);
    ...
    while (!futureResponses.empty()) {
        ...
        if (firstResp.zllStartCycle >= zllCycle) break;
        if (firstResp.ev) {
            if(firstResp.ev->isOffload()){
                if(firstResp.ev->getTid() == tid){
                    TRACE_MSG("linked Issue zll %ld with Resp zll %ld", zllCycle, firstResp.zllStartCycle);
                    firstResp.ev->addChild(ev, eventRecorder);
                    assert(maxCycle <= firstResp.zllStartCycle);
                    assert(firstResp.zllStartCycle >= lastEvProduced->zllStartCycle);
...
void OOOCoreRecorder::addOffloadIssueEvent(uint64_t evCycle, uint32_t tid){
    assert(lastEvProduced);
    uint64_t zllCycle = evCycle - gapCycles;
    assert_msg(zllCycle >= lastEvProduced->zllStartCycle, "zllCycle %ld last %ld", zllCycle, lastEvProduced->zllStartCycle);
    OOOIssueEvent* ev = new (eventRecorder) OOOIssueEvent(0, zllCycle, this, domain);
...
void OOOCoreRecorder::recordAccess(uint64_t curCycle, uint64_t dispatchCycle, uint64_t respCycle) {
    ...
    if (IsGet(tr.type)) {
        ...
        // if(IsOffload(tr.type))
        //     addOffloadIssueEvent(curCycle,tr.tid);
        // else
        //     addIssueEvent(curCycle, tr.tid);
        addIssueEvent(curCycle);
        ...
        if(IsOffload(tr.type)){
            ...
            for (FutureResponse& fr : GetPrioQueueContainer(futureResponses)) {
                if (fr.zllStartCycle < zllDispatchCycle && fr.ev) {
                    //offload can only be child of tasks/instrs in the same thread
                    if(fr.ev->getTid() == tr.tid){
        ...
        if(IsOffload(tr.type))
            offload = true;
        OOORespEvent *respEvent = new (eventRecorder) OOORespEvent(downDelay, this, tr.tid, offload, domain);
        respEvent->id = curId++;
        respEvent->setMinStartCycle(respCycle);
        tr.endEvent->addChild(respEvent, eventRecorder);
        // if(!IsOffload(tr.type)){//After dispatched , an instruction or offload task should not wait on an offload task's response
